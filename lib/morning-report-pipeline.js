...
import { assertLinePushQuota, pushLineMessages } from "./line-push.js";
...
function compactQuote(item) {
  const priceDigits = item?.label === "美債10年殖利率" ? 3 : 2;
  const format = (value, digits) => Number.isFinite(value)
  ...
  return {
    label: item?.label ?? "未知",
    symbol: item?.symbol ?? "",
    priceText: format(item?.price, priceDigits),
    changeText: Number.isFinite(item?.change) ? format(item.change, priceDigits) : "資料待補",
    changePercentText: Number.isFinite(item?.changePercent)
...
美國四大指數、重要科技股、商品與指標的數字只能逐字使用下方 JSON 的 priceText、changeText、changePercentText。寫著「資料待補」就照抄，不得自行補值、重新計算或混用不同交易日。新聞與今日事件可用 web search 補充，但需附來源名稱與日期。
...
export async function runMorningReportPipeline(now = new Date()) {
  ...
  try {
    ...
    line = { ok: false, error: error.message };
  }
