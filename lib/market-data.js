...
function normalizeQuote(raw, label) {
...
export async function fetchQuotes(symbolPairs) {
  ...
  return symbolPairs.map(([symbol, label]) => normalizeQuote(bySymbol.get(symbol), label));
}
...
export async function getMorningReportData() {
  ...
  const techAverage = tech.filter((q) => q?.changePercent != null).reduce((sum, q, _, arr) => sum + q.changePercent / arr.length, 0);
