...
function normalizeQuote(raw, label, fallbackSymbol = "") {
  ...
  return {
    ...
    change: Number.isFinite(raw?.regularMarketChange) ? raw.regularMarketChange : null,
...
async function fetchChartQuote(symbol, label) {
  try {
    ...
    const latestClose = validCloses.at(-1);
    const priorClose = validCloses.at(-2);
    const price = Number.isFinite(latestClose) ? latestClose : meta.regularMarketPrice;
    const previousClose = Number.isFinite(priorClose) ? priorClose : meta.previousClose;
