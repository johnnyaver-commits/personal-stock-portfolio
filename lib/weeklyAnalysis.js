...
function scoreHolding(holding, currencyTotal) {
  ...
  const reasons = [];
  if (pnlPercent > 30) reasons.push(`目前相對平均成本上漲 ${pnlPercent.toFixed(1)}%，漲幅偏大，避免追高`);
  else if (pnlPercent >= 8) reasons.push(`目前相對平均成本上漲 ${pnlPercent.toFixed(1)}%，趨勢偏強`);
  else if (pnlPercent <= -8) reasons.push(`目前相對平均成本下跌 ${Math.abs(pnlPercent).toFixed(1)}%，需等待止跌訊號`);
  else reasons.push("目前價格接近平均成本，方向尚未明確");
