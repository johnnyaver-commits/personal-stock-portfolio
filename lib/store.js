...
function summarizeHoldings(holdings) {
  return holdings.reduce(
    (summary, holding) => {
      ...
      summary[`${prefix}_market_value`] += Number(holding.market_value ?? 0);
      summary[`${prefix}_unrealized_pnl`] += Number(holding.unrealized_pnl ?? 0);
      return summary;
    },
    {
      twd_market_value: 0,
...
export async function listPortfolioTrends(ownerId = "all") {
  ...
  await ensureSchema();
  const rows = await getSql()`
    SELECT snapshot_date, owner_id, twd_market_value, usd_market_value, twd_unrealized_pnl, usd_unrealized_pnl
