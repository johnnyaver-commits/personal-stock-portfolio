...
import { latestCalendarMonths } from "@/lib/trendWindow";
import { currencyForSymbol, fetchYahooQuote, fetchYahooQuotes, normalizePortfolioSymbol } from "@/lib/yahooFinance";

const LEGACY_DEFAULT_OWNERS = [
...
let memoryTrendSnapshots = [];
let schemaReady = false;

const TREND_BASELINE_DATE = "2026-06-05";
...
function trendOwnerId(ownerId = "all") {
  return ownerId === "all" || ownerId == null ? 0 : normalizeOwnerId(ownerId);
}

function toHolding(row) {
  const symbol = normalizePortfolioSymbol(row.symbol, row.market);
  const currency = currencyForSymbol(symbol, row.market);
  return {
    ...row,
    id: Number(row.id),
...
function toTrendSnapshot(row) {
  ...
  return {
    ...
    twd_market_value: Number(row.twd_market_value ?? 0),
    usd_market_value: Number(row.usd_market_value ?? 0),
    twd_cost_basis: Number(row.twd_cost_basis ?? Number(row.twd_market_value ?? 0) - Number(row.twd_unrealized_pnl ?? 0)),
    usd_cost_basis: Number(row.usd_cost_basis ?? Number(row.usd_market_value ?? 0) - Number(row.usd_unrealized_pnl ?? 0)),
    twd_unrealized_pnl: Number(row.twd_unrealized_pnl ?? 0),
    usd_unrealized_pnl: Number(row.usd_unrealized_pnl ?? 0)
  };
}
...
function summarizeHoldings(holdings) {
  return holdings.reduce(
    (summary, holding) => {
      const prefix = holding.currency === "TWD" ? "twd" : "usd";
      const marketValue = Number(holding.market_value ?? 0);
      const costBasis = Number(holding.cost_basis ?? 0);
      summary[`${prefix}_market_value`] += marketValue;
      summary[`${prefix}_cost_basis`] += costBasis;
      summary[`${prefix}_unrealized_pnl`] += marketValue - costBasis;
      return summary;
    },
    {
      twd_market_value: 0,
      ...
      twd_cost_basis: 0,
      usd_cost_basis: 0,
      twd_unrealized_pnl: 0,
      usd_unrealized_pnl: 0
    }
  );
}

function latestByPeriod(rows, period) {
  const grouped = new Map();

  for (const row of rows) {
    const key = period === "year" ? row.snapshot_date.slice(0, 4) : row.snapshot_date.slice(0, 7);
    const existing = grouped.get(key);
    if (!existing || row.snapshot_date > existing.snapshot_date) grouped.set(key, row);
  }

  return [...grouped.values()].sort((a, b) => a.snapshot_date.localeCompare(b.snapshot_date));
}

function buildTrendResponse(rows) {
  const ordered = rows
    .map(toTrendSnapshot)
    .filter((row) => row.snapshot_date >= TREND_BASELINE_DATE)
    .sort((a, b) => a.snapshot_date.localeCompare(b.snapshot_date));
  return {
    daily: latestCalendarMonths(ordered, 6),
    monthly: latestByPeriod(ordered, "month").slice(-12),
    yearly: latestByPeriod(ordered, "year").slice(-5)
  };
}
...
async function ensureSchema() {
  ...
  await sql`
    ...
    SET
      ...
      currency = 'TWD',
      updated_at = NOW()
    WHERE symbol ~ '^[0-9]{4,6}$'
      AND NOT EXISTS (
        SELECT 1
        FROM portfolio_holdings existing
        WHERE existing.owner_id = portfolio_holdings.owner_id
          AND existing.symbol = portfolio_holdings.symbol || '.TW'
      )
  `;
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS portfolio_summary_snapshots (
      id BIGSERIAL PRIMARY KEY,
      snapshot_date DATE NOT NULL,
      owner_id BIGINT NOT NULL DEFAULT 0,
      twd_market_value NUMERIC(20, 6) NOT NULL DEFAULT 0,
      usd_market_value NUMERIC(20, 6) NOT NULL DEFAULT 0,
      twd_cost_basis NUMERIC(20, 6) NOT NULL DEFAULT 0,
      usd_cost_basis NUMERIC(20, 6) NOT NULL DEFAULT 0,
      twd_unrealized_pnl NUMERIC(20, 6) NOT NULL DEFAULT 0,
      usd_unrealized_pnl NUMERIC(20, 6) NOT NULL DEFAULT 0,
      created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      UNIQUE (snapshot_date, owner_id)
    )
  `;
  await sql`ALTER TABLE portfolio_summary_snapshots ADD COLUMN IF NOT EXISTS twd_cost_basis NUMERIC(20, 6) NOT NULL DEFAULT 0`;
  await sql`ALTER TABLE portfolio_summary_snapshots ADD COLUMN IF NOT EXISTS usd_cost_basis NUMERIC(20, 6) NOT NULL DEFAULT 0`;
  await sql`
    UPDATE portfolio_summary_snapshots
    SET
      twd_cost_basis = twd_market_value - twd_unrealized_pnl,
      usd_cost_basis = usd_market_value - usd_unrealized_pnl,
      updated_at = NOW()
    WHERE twd_cost_basis = 0
      AND usd_cost_basis = 0
      AND (twd_market_value <> 0 OR usd_market_value <> 0 OR twd_unrealized_pnl <> 0 OR usd_unrealized_pnl <> 0)
  `;
...
async function saveTrendSnapshots(holdings) {
  const today = new Date().toISOString().slice(0, 10);
  const snapshots = [{ owner_id: 0, ...summarizeHoldings(holdings) }];

  for (const owner of DEFAULT_OWNERS) {
    snapshots.push({
      owner_id: owner.id,
      ...summarizeHoldings(holdings.filter((holding) => holding.owner_id === owner.id))
    });
  }

  if (!hasDatabaseUrl()) {
    for (const snapshot of snapshots) {
      const index = memoryTrendSnapshots.findIndex((item) => item.snapshot_date === today && item.owner_id === snapshot.owner_id);
      const nextSnapshot = { snapshot_date: today, ...snapshot };
      if (index >= 0) memoryTrendSnapshots[index] = nextSnapshot;
      else memoryTrendSnapshots.push(nextSnapshot);
    }
    return;
  }
  ...
  for (const snapshot of snapshots) {
    await sql`
      INSERT INTO portfolio_summary_snapshots (
        snapshot_date,
        owner_id,
        twd_market_value,
        usd_market_value,
      ...
      VALUES (
        ${today},
        ${snapshot.owner_id},
        ${roundMoney(snapshot.twd_market_value)},
        ${roundMoney(snapshot.usd_market_value)},
        ${roundMoney(snapshot.twd_cost_basis)},
        ${roundMoney(snapshot.usd_cost_basis)},
        ${roundMoney(snapshot.twd_unrealized_pnl)},
        ${roundMoney(snapshot.usd_unrealized_pnl)}
      )
      ON CONFLICT (snapshot_date, owner_id) DO UPDATE SET
        twd_market_value = EXCLUDED.twd_market_value,
        usd_market_value = EXCLUDED.usd_market_value,
        twd_cost_basis = EXCLUDED.twd_cost_basis,
        usd_cost_basis = EXCLUDED.usd_cost_basis,
        twd_unrealized_pnl = EXCLUDED.twd_unrealized_pnl,
        usd_unrealized_pnl = EXCLUDED.usd_unrealized_pnl,
        updated_at = NOW()
    `;
  }
}

export async function listPortfolioTrends(ownerId = "all") {
  const holdings = await listHoldings();
  await saveTrendSnapshots(holdings);
  const selectedOwnerId = trendOwnerId(ownerId);

  if (!hasDatabaseUrl()) {
    return buildTrendResponse(memoryTrendSnapshots.filter((snapshot) => snapshot.owner_id === selectedOwnerId));
  }

  await ensureSchema();
  const rows = await getSql()`
    SELECT snapshot_date, owner_id, twd_market_value, usd_market_value, twd_cost_basis, usd_cost_basis, twd_unrealized_pnl, usd_unrealized_pnl
    FROM portfolio_summary_snapshots
    WHERE owner_id = ${selectedOwnerId}
    ORDER BY snapshot_date ASC
  `;
  return buildTrendResponse(rows);
}
...
export async function createHolding(data) {
  const symbol = normalizePortfolioSymbol(data.symbol, data.market);
  ...
  const holdingData = {
    ...
    currency: currencyForSymbol(symbol, data.market)
  };
...
export async function updateHolding(id, data) {
  if (!hasDatabaseUrl()) {
    ...
    memoryHoldings[index] = {
      ...
      owner_id: ownerId,
      owner_name: ownerNameForId(ownerId),
      symbol: normalizePortfolioSymbol(data.symbol ?? memoryHoldings[index].symbol, data.market),
      currency: currencyForSymbol(data.symbol ?? memoryHoldings[index].symbol, data.market),
  ...
  const symbol = normalizePortfolioSymbol(merged.symbol, merged.market);
  const [row] = await getSql()`
    UPDATE portfolio_holdings
    SET
      ...
      currency = ${currencyForSymbol(symbol, merged.market)},
      updated_at = NOW()
    WHERE id = ${Number(id)}
    RETURNING id
  `;
