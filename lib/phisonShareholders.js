...
function toTrendRow(row) {
  return {
    ...
    total_holders: row.total_holders == null ? null : Number(row.total_holders),
    total_shares: row.total_shares == null ? null : Number(row.total_shares),
...
export async function listPhisonShareholderTrend() {
  ...
  const rows = await getSql()`
    SELECT symbol, name, snapshot_date, large_holders, large_shares, large_percentage, total_holders, total_shares, source, frequency
