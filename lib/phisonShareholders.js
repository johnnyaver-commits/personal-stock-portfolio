...
const FUBON_INSTITUTIONAL_URL = "https://fubon-ebrokerdj.fbs.com.tw/z/zc/zcl/zcl.djhtm";
...
function parseInstitutionalRows(html) {
  return [...html.matchAll(/<tr[^>]*>\s*([\s\S]*?)<\/tr>/gi)]
    .map((rowMatch) => [...rowMatch[1].matchAll(/<td[^>]*>\s*([\s\S]*?)\s*<\/td>/gi)].map((cell) => cell[1].replace(/<[^>]+>/g, "").replace(/&nbsp;/g, "").trim()))
    .filter((cells) => /^\d{3}\/\d{2}\/\d{2}$/.test(cells[0]) && cells.length >= 11)
    .map((cells) => ({
      snapshot_date: rocDateToIso(cells[0]),
      foreign_buy_sell_lots: numeric(cells[1]),
      investment_trust_buy_sell_lots: numeric(cells[2]),
      dealer_buy_sell_lots: numeric(cells[3]),
      institutional_buy_sell_lots: numeric(cells[4]),
      foreign_holding_lots: numeric(cells[5]),
      investment_trust_holding_lots: numeric(cells[6]),
      dealer_holding_lots: numeric(cells[7]),
      institutional_holding_lots: numeric(cells[8]),
      foreign_holding_percentage: numeric(cells[9].replace("%", "")),
      investment_trust_holding_percentage: Number(((numeric(cells[6]) / numeric(cells[8])) * numeric(cells[10].replace("%", ""))).toFixed(4)),
      dealer_holding_percentage: Number(((numeric(cells[7]) / numeric(cells[8])) * numeric(cells[10].replace("%", ""))).toFixed(4)),
      institutional_holding_percentage: numeric(cells[10].replace("%", ""))
    }));
}

async function fetchPhisonInstitutionalTrend(startDate = "2026-06-05") {
  if (institutionalCache && Date.now() - institutionalCache.createdAt < CACHE_TTL_MS) return institutionalCache.data;

  const end = isoToQueryDate(new Date().toISOString().slice(0, 10));
  const start = isoToQueryDate(startDate);
  const response = await fetch(`${FUBON_INSTITUTIONAL_URL}?a=${PHISON_SYMBOL}&c=${encodeURIComponent(start)}&d=${encodeURIComponent(end)}`, {
    cache: "no-store",
    headers: {
      accept: "text/html,*/*",
  ...
  const data = parseInstitutionalRows(html).sort((a, b) => a.snapshot_date.localeCompare(b.snapshot_date));
  institutionalCache = { createdAt: Date.now(), data };
  return data;
}

function mergeInstitutionalTrend(trend, institutionalTrend) {
  const institutionalByDate = new Map(institutionalTrend.map((point) => [point.snapshot_date, point]));
  return trend.map((point) => ({
    ...point,
    ...(institutionalByDate.get(point.snapshot_date) ?? {})
  }));
}
...
export async function listPhisonShareholderTrend() {
  ...
  const institutionalTrend = await fetchPhisonInstitutionalTrend();

  if (!hasDatabaseUrl()) {
    const trend = mergeInstitutionalTrend([latest], institutionalTrend);
    return { latest: trend.at(-1) ?? latest, trend, institutional_trend: institutionalTrend };
  }
  ...
  const mergedTrend = mergeInstitutionalTrend(trend, institutionalTrend);
  return { latest: mergedTrend.at(-1) ?? latest, trend: mergedTrend, institutional_trend: institutionalTrend };
}
